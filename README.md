...
For example, `ghcr.io/bylocico/aws-ses-v2-local:2.10.0` contains `aws-ses-v2-local@2.10.0`.
...
```bash
docker pull ghcr.io/bylocico/aws-ses-v2-local:2.10.0
docker run -p 8005:8005 ghcr.io/bylocico/aws-ses-v2-local:2.10.0
```
...
services:
  ses-local:
    image: ghcr.io/bylocico/aws-ses-v2-local:2.10.0
    ports:
...
```bash
node scripts/update-version.mjs --version 2.10.0
```
...
The workflow builds multi-arch images (`linux/amd64`, `linux/arm64`) and pushes to GHCR with version tags (`2.10.0`, `2.9`, `2`, `latest`).

### Release checklist

1. Confirm the npm version exists: `npm view aws-ses-v2-local@2.10.0 version`
2. Update `VERSION`, `Dockerfile`, and `README.md` (or run `node scripts/update-version.mjs --version X.Y.Z`)
3. Push to `main`
4. Create a GitHub Release with tag `v2.10.0`
